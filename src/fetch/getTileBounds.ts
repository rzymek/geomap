...
function getTileBounds(source:TileSource, box:Box, tileZ:number) {
    const scaleDenominators = source.tiles.EPSG_2180.map(function (it) {
        return it.scaleDenominator;
    });
    const matrixSizes = source.tiles.EPSG_2180.map(function (it) {
