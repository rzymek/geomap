...
export class MapGrid extends React.Component<MapGridProps, {}> {
    ...
    private getGridMercatorRotation(utmGrid: CoordinatesXY[]): string {
        const {box} = this.props;
        const zone = utmZone(puwg2ll([box.x1, box.y1]));
        const topLeftUTM = utmGrid[3];
        const topLeftLL = utm2ll(zone, topLeftUTM);
        const bottomLeftLL = utm2ll(zone, utmGrid[0]);
        const bottomLeftAligned = {
            x: topLeftLL.x,
            y: bottomLeftLL.y
        };
        const bottomLeftAlignedUTM = ll2utm(zone, bottomLeftAligned);
        const angle = angleDeg(
            this.utmToPx(topLeftUTM),
            this.utmToPx(bottomLeftAlignedUTM),
        );
